...
- *Score*: commands for scoring the current entry or selection
  (~elfeed-ai-score~) and for scoring unscored entries
  (~elfeed-ai-score-unscored~).

- *Display*: toggles for sort order (~elfeed-ai-toggle-sort~) and the
  minor mode (~elfeed-ai-mode~).

- *Scoring*: interactive adjustment of the AI model, auto-scoring,
  summary generation, relevance threshold, max content length, and the
  number of days for ~elfeed-ai-score-unscored~.

- *Budget*: toggle between token and dollar budget types, and set the
  budget limit.  In dollar mode, the group heading displays the total
  cost of the last 100 scored entries, helping you calibrate the
  daily limit.
